...
より詳細な API の仕様やユーティリティの活用方法については、以下のドキュメントも参照してください。

- **[ST7789V ドライバ・マニュアル](./docs/ST7789V-manual.md)**: `ST7789V` クラスの詳細な使い方と設定方法。
- **[パフォーマンスコア・マニュアル](./docs/performance_core_manual.md)**: `ColorConverter` など、描画の高速化に寄与する内部ユーティリティの解説。
...
- **対話型ウィザードによる自動設定 (推奨)**:
  画面を見ながらキー操作（向き、色反転、BGR順序、表示位置の微調整）を行い、最適な設定を特定して `pi0disp.toml` に保存します。
  ```sh
  uv run pi0disp lcd-check --wizard
  ```
  操作キー: `a-d`(向き), `i`(反転), `g`(BGR), `hjkl`(位置微調整), `ENTER`(保存)
