...
def test_determine_lcd_settings():
    ...
    assert determine_lcd_settings(False, False, "red", "black") == (False, False)
    
    # ソフトウェアがRGB/Inv:Offで送っていて、ユーザーが青/黒に見えているなら、パネルはBGR/Inv:Off
    assert determine_lcd_settings(False, False, "blue", "black") == (True, False)
    
    # ソフトウェアがRGB/Inv:Offで送っていて、ユーザーがシアン/白に見えているなら、パネルはRGB/Inv:On
    assert determine_lcd_settings(False, False, "cyan", "white") == (False, True)
    
    # ソフトウェアがRGB/Inv:Offで送っていて、ユーザーが水色(イエローの反対など)/白に見えているなら...
    # (反転とBGRが両方起きているケース)
    # BGRで送った赤は「青」になり、それが反転すると「黄色(Yellow)」になる
    assert determine_lcd_settings(False, False, "yellow", "white") == (True, True)
