...
def test_set_window_with_offset(mock_pi_instance):
    ...
    # When MV=1 (Landscape):
    #   CASET should get Logical X (0, 100) + y_offset (20) -> [0, 20, 0, 120]
    #   RASET should get Logical Y (0, 100) + x_offset (10) -> [0, 10, 0, 110]
    # When MV=0 (Portrait):
    #   CASET should get Logical X (0, 100) + x_offset (10) -> [0, 10, 0, 110]
    #   RASET should get Logical Y (0, 100) + y_offset (20) -> [0, 20, 0, 120]
    ...
    assert caset_data_found, (
        f"CASET data {expected_caset} was not written. MV={disp._mv}, Calls: {calls}"
    )
    assert raset_data_found, (
        f"RASET data {expected_raset} was not written. MV={disp._mv}, Calls: {calls}"
    )


def test_set_window_rotation_90(mock_pi_instance):
    """Test that set_window maps logical X to CASET when rotation is 90 (MV=1)."""
    disp = ST7789V(
        pin=SpiPins(rst=25, dc=24, bl=23),
        size=DispSize(240, 320),
        rotation=90,  # MV=1
    ...
    # Since MV=1 (Landscape):
    # CASET should get Logical X (0, 319)
    # RASET should get Logical Y (0, 239)
    ...
    assert caset_x_found, (
        f"CASET should receive X coordinates when MV=1. Calls: {calls}"
    )
    assert raset_y_found, (
        f"RASET should receive Y coordinates when MV=1. Calls: {calls}"
    )
