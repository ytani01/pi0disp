...
- [x] タスク: 実装をリファクタリングする (Optional)。
- [x] タスク: テストカバレッジを確認する。
- [x] タスク: フェーズ完了: Conductor - ユーザー手動検証 '問題特定とテスト駆動開発 (TDD)' (workflow.mdのプロトコル)

## 3. フェーズ: 汎用性と堅牢性の向上 [checkpoint: 37f778c]

### 目標
データシートとの詳細な比較に基づき、ドライバの汎用性と堅牢性をさらに向上させる。

### タスク
- [x] タスク: データシートに記載されているが、現在の実装で不足している、または不適切に使用されているコマンドや設定を確認する。
  - [x] サブタスク: 特に、色の順序 (`BGR`)、色反転 (`INVON`/`INVOFF`)、ガンマ補正などのオプション設定を見直す。
- [x] タスク: 不足しているテストケースを追加する。
  - [x] サブタスク: `BGR`、`INVON`/`INVOFF`の切り替え機能に関するテストを作成する。
- [x] タスク: 汎用性向上のためのオプションを追加する (Green Phase)。
  - [x] サブタスク: `__init__`に`invert: bool = True`と`bgr: bool = True`パラメータを追加する。
  - [x] サブタスク: `CMD`辞書に`INVOFF`を追加する。
  - [x] サブタスク: `init_display()`で`invert`フラグに応じて`INVON`/`INVOFF`を切り替えるようにする。`SLPOUT`後の遅延を120msに短縮する。
  - [x] サブタスク: `set_rotation()`で`bgr`フラグに応じて`MADCTL`の値を調整するようにする。
- [x] タスク: 実装をリファクタリングする (Optional)。
- [x] タスク: テストカバレッジを確認する。
- [x] タスク: フェーズ完了: Conductor - ユーザー手動検証 '汎用性と堅牢性の向上' (workflow.mdのプロトコル)
