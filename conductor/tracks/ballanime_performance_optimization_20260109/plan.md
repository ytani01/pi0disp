...
- [ ] テスト: `tests/test_ballanime_cmd.py` で `--mode` オプションのパースと、各モードへの分岐が正しく行われることをテストする。
- [ ] 実装: `--mode` オプションを追加し、既存ロジックを `simple` モードとして隔離する。
- [ ] 検証: 実際にコマンドを叩き、`simple` モードが従来の挙動（全画面更新）を維持していることをログで確認する。
