...
使用しているパネルに最適な `invert` と `bgr` 設定を特定するためのツールです。
CLIツールとして提供されており、対話形式で設定を特定できる「ウィザード」モードが推奨されます。
...
>
> **最適化機能 (Dirty Rectangle):**
> 前回の表示イメージと比較し、差分のある最小矩形領域（Dirty Rectangle）のみを自動的に計算して更新します。これにより、表情アニメーションのように画像の一部のみが変化する場合、通信データ量と CPU 負荷を劇的に削減できます。

**`display_region(image: Image.Image, x0, y0, x1, y1)`**

> 指定した矩形領域（左上 `x0, y0` から右下 `x1, y1`）のみを更新します。
> 手動で更新範囲を制御したい場合に使用しますが、通常は `display()` メソッドによる自動差分更新で十分なパフォーマンスが得られます。

---

## パフォーマンス最適化技術

`pi0disp` は、Raspberry Pi Zero 2W のような限られたリソースで最高のパフォーマンスを発揮するために、以下の最適化技術を組み込んでいます。

1. **自動差分更新 (Dirty Rectangle)**:
   ピクセル単位での変更を検出し、変化があった領域のみを SPI 送信します。
2. **GPIO 状態キャッシュ**:
   DC (Data/Command) ピンなどの制御信号の状態を保持し、冗長な GPIO 操作（通信）を排除します。
3. **適応型チャンク送信 (Adaptive Chunking)**:
   ネットワークの状態や通信速度に合わせて、最適なデータサイズ（最大 32KB）で SPI 送信を行います。
4. **NumPy による高速処理**:
   色変換（RGB to RGB565）や差分計算に NumPy を使用し、Python レベルのオーバーヘッドを最小化しています。
