...
## Cairoとは？
Cairo（カイロ）は、2Dグラフィックスを美しく描画するためのライブラリです。
このプロジェクトでは、主に以下の目的で使用されています：
- **アンチエイリアス**: 図形の縁にあるギザギザ（ジャギー）をなくし、滑らかな見た目にします。
- **高品質な描画**: 標準の描画機能（PIL）よりも、円や斜め線を綺麗に描くことができます。
- **表現力**: 透過処理や複雑な図形の組み合わせを、より直感的に美しく処理できます。

低解像度のディスプレイでは、この Cairo による「滑らかさ」が視覚的なクオリティを大きく左右します。
...
また、本検証を通じて **「現在のST7789Vドライバ（pigpiod経由）の全画面転送能力が非常に高い」** ことも浮き彫りになりました。320x240 サイズであれば、全画面転送でも十分なパフォーマンスが得られるため、無理に部分更新を追求するよりも、Cairo による「描画の美しさ」に計算リソースを割り振る方が、総合的な満足度が高くなるという興味深い結果となりました。
