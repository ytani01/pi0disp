...
def create_test_pattern(width, height, rotation):
    ...
    for x in range(width):
        ...
        g = int(x * 255 / width)
        draw.line([x, y_start + grad_h // 3, x, y_start + (grad_h // 3) * 2], fill=(0, g, 0))
        b = int(x * 255 / width)
        draw.line([x, y_start + (grad_h // 3) * 2, x, y_start + grad_h], fill=(0, 0, b))

    # 3. Central Info Box
    center_x, center_y = width // 2, height // 2
    box_w, box_h = 160, 80
    draw.rectangle([center_x - box_w//2, center_y - box_h//2, 
                    center_x + box_w//2, center_y + box_h//2], 
                   outline="white", fill=(40, 40, 40))
    ...
    for i, line in enumerate(text_lines):
        draw.text((center_x - 70, center_y - 30 + i * 20), line, fill="white", font=font)
    ...
    draw.ellipse([width - 25, height - 25, width - 5, height - 5], fill="magenta") # Bottom-Right
